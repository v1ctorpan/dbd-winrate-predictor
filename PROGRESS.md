...
- **注意**：BV16 的 10s 稀疏帧恰抓到 p1"刚逃出高亮"瞬间，现分类器对该高亮图标（escaped ms≈0.49、executed≈0.55）会漏判；0.5s 全量管线因有后续稳定帧不受影响。高亮逃出模板待补（需用户协助截图确认）。
...
- **注意**：BV1Uu 为 10s 稀疏帧，重分段后两局自动推断均得 0（帧内从未识别到 escaped），与 match_2 真值 2 冲突——属同一"10s 稀疏/高亮逃出漏判"问题；该旧帧目录可能也未覆盖到真正逃生时刻。需后续用源视频 0.5s 重抽才可靠（当前无 BV1Uu mp4）。
- 全量回归：**107 tests PASS，约 55s**。
...
**main（当前分支）**：Task1-4 + Task5 修复 + UT 提速(§3.15) + BV1QUt 验证驱动修复与并行产线(§3.16) + BV1pht 真值纠正(§3.19) + 自动锚点/切局(§3.21) + 结局自动标注(§3.22) 均已推送。dataset 现 7 行（BV1Uu 2 局、BV16=3、BV1pht=1、BV1QUt=0/0；BV1aat=-1）。
